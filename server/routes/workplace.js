...
// GET /api/workplace?staffCode=&date=&from=&to= — public, no auth (mirrors the old
// read?sheet=WorkPlace, which had no server-side auth either).
router.get('/', asyncHandler(async (req, res) => {
  ...
  const { rows } = await pool.query(
    `SELECT * FROM work_place_reports ${where} ORDER BY date DESC, id DESC`,
    params
  );
  res.json({ status: 'ok', data: rows });
}));

// POST /api/workplace — create. Public, no auth: staff-portal.html's submitReport()
...
router.post('/', asyncHandler(async (req, res) => {
  ...
  const { rows } = await pool.query(
    `INSERT INTO work_place_reports (${COLUMNS.join(', ')}) VALUES (${placeholders}) RETURNING id`,
    values
  );
...
router.put('/:id', asyncHandler(async (req, res) => {
  ...
  const result = await pool.query(`UPDATE work_place_reports SET ${setClause} WHERE id = $1`, values);
  if (result.rowCount === 0) return res.status(404).json({ status: 'error', msg: 'Workplace report not found: ' + id });
  res.json({ status: 'ok' });
}));

// DELETE /api/workplace/:id
router.delete('/:id', asyncHandler(async (req, res) => {
  const result = await pool.query('DELETE FROM work_place_reports WHERE id = $1', [req.params.id]);
  if (result.rowCount === 0) return res.status(404).json({ status: 'error', msg: 'Workplace report not found: ' + req.params.id });
  res.json({ status: 'ok' });
}));
